...
const Habits = (props) => {
  ...
  const handleSubmit = () => {
    ...
    const reminderDate = new Date(
      ...
      null,
      null,
      reminder.substring(0, 2),
      reminder.substring(3, 5)
    );
    // if existing habit
    ...
    fetch("http://localhost:8080/api/habit", requestOptions)
      .then((response) => response.json())
      .then((val) => this.setState({ postId: val.id }))
      .catch((error) => {
        this.setState({ errorMessage: error.toString() });
        console.error("There was an error in uploading your habits!", error);
      });
  };
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={10}>
          <Typography variant="body1" className={classes.instructions}>
            Choose a habit you want to improve on from the list. If it's not
            there, type it in and create one!
          </Typography>
          <Autocomplete
            renderInput={(params) => (
              <TextField {...params} label="Choose or Create a Habit!" />
            )}
            options={habits}
            onChange={(event) => {
        ...
        <Grid item xs={10}>
          <Typography variant="body1" className={classes.instructions}>
            What is this habit? Explain it in a few words. Leave this empty if
            you chose an existing habit.
          </Typography>
          <TextField
            label="Habit Explanation"
            onChange={(event) => setNewHabitExplanation(event.target.value)}
          />
        </Grid>
        <Grid item xs={1} />
        <Grid item xs={1} />
